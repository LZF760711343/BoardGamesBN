...
namespace EffectUtils {
    ...
    //     return tween;
    // }
    // export function showRightToLeft(object: egret.DisplayObject,cb?: Function,target?,data?,delay: number = 1000): egret.Tween {
    ...
    //             .wait(delay)
    // //            .to({ x: (width - object.width) / 2 - 100 },500)
    //             .to({ x: - object.width },300,egret.Ease.sineIn);
    //         if(cb) {
    //             tween = tween.call(cb,target,data);
    //         }
    //         return tween;
    ...
    //obj         闪烁对象
    //loop    是否循环
    export function blinkEffect(obj: egret.DisplayObject, loop: boolean = false) {
        //    new BitmapBlink(obj,interval); 
        obj.alpha = 0;
        return egret.Tween.get(obj, { loop: loop })
            .to({ alpha: 1 }, 200)
            .to({ alpha: 0 }, 200);
